...
public class ClassManager {
    ...
    private void tickGojo(Player p, UUID uuid) {
        ...
        // Six Eyes: Speed I + Night Vision (sees in the dark, perceives everything)
        applyPermanent(p, PotionEffectType.SPEED, 0);
        applyPermanent(p, PotionEffectType.NIGHT_VISION, 0);
    ...
    private void tickLight(Player p, UUID uuid) {
        ...
        // Yagami can wear a helmet — bonus: Absorption I while helmeted
        if (p.getInventory().getHelmet() != null) {
            applyPermanent(p, PotionEffectType.ABSORPTION, 0);
        } else {
    ...
    private void tickLebron(Player p, UUID uuid) {
        ...
        applyPermanent(p, PotionEffectType.JUMP_BOOST, 3); // Jump Boost IV — +1 more block
    }
    ...
    public void updateCooldownDisplay(Player player) {
        ...
        switch (clazz) {
            ...
            case WUKONG ->
                bar += "§fNimbus: " + cd.getFormattedRemaining(uuid, "nimbus_cloud")
                     + "  §fInvis: " + cd.getFormattedRemaining(uuid, "wukong_invis");
            case L ->
