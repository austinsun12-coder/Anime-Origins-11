...
public class AnimeClassesPlugin extends JavaPlugin {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (cmd.getName().equalsIgnoreCase("animeclass")) {
            ...
            // Non-admins can't reselect once they have a class
            if (classManager.hasClass(p) && !p.isOp() && !p.hasPermission("animeclasses.admin")) {
                AnimeClass current = classManager.getClass(p);
                p.sendMessage("§cYou already have a class: " + current.getColoredName());
                p.sendMessage("§7Ask an admin to reset it with §e/resetclass " + p.getName());
                return true;
            }
            selectionGUI.open(p);
            return true;
